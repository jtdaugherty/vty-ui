...
The \fw{vBox} function returns a new widget of type \fw{Widget (Box a
  b)} which lays out its two children vertically in the order they are
passed to the function.  The \fw{hBox} function does the same for
...
\fw{IO} arguments; so you can use them to create nested boxes as
follows:
...
wrap it with \fw{return} to use it with these combinators:
...
space wisely, and to determine how to use it, \fw{vty-ui} requires
...
terminal.  See the API documentation for the \fw{Fills} module in
particular.
