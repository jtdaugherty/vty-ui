...
desired width, \fw{clip1d}.  These functions exist because, in
...
\fw{textWidth} function returns a \fw{Phys} value which indicates the
\textit{physical width} of its input, in columns, which may be more
than its \textit{logical character width} in characters.  The
\fw{clip1d} function returns a text string truncated so that its
characters fit into the specified physical space.  It also returns
\fw{Bool} indicators so you can tell whether clipping occurred in the
...
always using \fw{textWidth} and \fw{clip1d} in case we need to start
